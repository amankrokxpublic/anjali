var w = c.width = window.innerWidth,
		...
		opts = {
			strings: [ 'HAPPY', 'BIRTHDAY!','to my Love Anjali !' ],
			charSize: 30,
			charSpacing: 32,
			lineHeight: 40,
...
function anim(){
	...
	if( done ) {
		...
		if(ctr == (quotes.length -1)) {ctr = 0;}
		for( var i = 0; i < quotes[ctr].length; ++i ){
			for( var j = 0; j < quotes[ctr][ i ].length; ++j ){
				letters.push( new Letter( quotes[ctr][ i ][ j ], 
					j * opts.charSpacing + opts.charSpacing / 2 - quotes[ctr][ i ].length * opts.charSize / 2,
					i * opts.lineHeight + opts.lineHeight / 2 - quotes[ctr].length * opts.lineHeight / 2 ) );
				}
			}
		}
}
